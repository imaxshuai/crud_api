...
namespace Max\CrudApi\Http\Models;

use Max\CrudApi\Http\Traits\Columns;
use Max\CrudApi\Http\Traits\Fields;
use Max\CrudApi\Http\Traits\Query;
use Max\CrudApi\Http\Traits\Buttons;
use Max\CrudApi\Http\Traits\Filters;
use Max\CrudApi\Http\Traits\Validation;
use Max\CrudApi\Http\Traits\Operations;
use Max\CrudApi\Http\Traits\Settings;

class CrudPanel
{
    // load all the default CrudPanel features
    use  Columns, Fields, Query, Buttons, Filters, Validation, Operations, Settings;
            ...
            if (is_array($entries)) {
                ...
                if (!isset($entries[$attribute])) {
                    ...
                    if ($model_instance->hasGetMutator($attribute) && isset($entries[$modelKey])) {
                        ...
                        $attributes[$entry_in_database->{$modelKey}] = $this->parseTranslatableAttributes($model_instance,
                            $attribute, $entry_in_database->{$attribute});
                    } else {
                        //we have multiple entries
                        //for each entry we check if $attribute exists in array or try to check if it's an acessor.
                        foreach ($entries as $entry) {
                            if (isset($entry[$attribute])) {
                                $attributes[$entry[$modelKey]] = $this->parseTranslatableAttributes($model_instance,
                    ...
                    $attributes[$entries[$modelKey]] = $this->parseTranslatableAttributes($model_instance, $attribute,
                        $entries[$attribute]);
                }
            }
        }
            ...
            if (!empty($relationArray)) {
                foreach ($currentResults as $currentResult) {
                    $results = array_merge_recursive($results,
                        $this->getRelatedEntries($currentResult, implode('.', $relationArray)));
                }
            } else {
